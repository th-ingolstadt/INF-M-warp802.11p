...
function interleaver_ram(this_block)

  this_block.setTopLevelLanguage('Verilog');

  this_block.setEntityName('interleaver_ram');
  ...
  douta_port.setType('UFix_8_0');
  doutb_port = this_block.port('doutb');
  doutb_port.setType('UFix_8_0');
  ...
  if (this_block.inputTypesKnown)
    ...
    if (this_block.port('dina').width ~= 1);
      this_block.setError('Input data type for port "dina" must have width=1.');
    end
    ...
    if (this_block.port('dinb').width ~= 1);
      this_block.setError('Input data type for port "dinb" must have width=1.');
    end
  ...
  %    this_block.addFile('');
  %    this_block.addFile('');
  this_block.addFile('blackboxes/dp_ram_wr_1b_rd_8b_512b.ngc');
  this_block.addFile('blackboxes/interleaver_ram.v');
