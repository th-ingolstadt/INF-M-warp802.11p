function interleave_mapping_ROM = wlan_tx_interleave_rom_gen()
...
%  -28 for 11n. The Tx PHY IFFT core requires inputs in natual order,
%  starting at subcarrier 0 (DC), then +1, +2, etc. Thus the interleaving
...
% The Tx PHY interleaver uses a single RAM to store all the bits for a
% single OFDM symbol. The same RAM is used for all rates. The RAM stores
% coded/punctured data bits for each subcarrier on 8-bit boundaries. By
% allocating 1 byte per subcarrier the design can use an efficient BRAM
% architecture with 2 parallel 1-bit writes (coded bits A/B) and serial
% 8-bit reads (bits for 1 subcarrier). Even with the aspect ratio change
...
% output index values to place each subcarrier's bits on an 8-bit boundary.
%
...
interleave_mapping_ROM = [];
interleave_mapping_ROM = [interleave_mapping_ROM   ramaddr_11a_BPSK    zeros(1, 512-length(ramaddr_11a_BPSK))];
interleave_mapping_ROM = [interleave_mapping_ROM   ramaddr_11a_QPSK    zeros(1, 512-length(ramaddr_11a_QPSK))];
interleave_mapping_ROM = [interleave_mapping_ROM   ramaddr_11a_16QAM   zeros(1, 512-length(ramaddr_11a_16QAM))];
interleave_mapping_ROM = [interleave_mapping_ROM   ramaddr_11a_64QAM   zeros(1, 512-length(ramaddr_11a_64QAM))];
interleave_mapping_ROM = [interleave_mapping_ROM   ramaddr_11n_BPSK    zeros(1, 512-length(ramaddr_11n_BPSK))];
interleave_mapping_ROM = [interleave_mapping_ROM   ramaddr_11n_QPSK    zeros(1, 512-length(ramaddr_11n_QPSK))];
interleave_mapping_ROM = [interleave_mapping_ROM   ramaddr_11n_16QAM   zeros(1, 512-length(ramaddr_11n_16QAM))];
interleave_mapping_ROM = [interleave_mapping_ROM   ramaddr_11n_64QAM   zeros(1, 512-length(ramaddr_11n_64QAM))];
