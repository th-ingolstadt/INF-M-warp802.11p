...
function nodesInfo = wl_nodesConfig(varargin)
    ...
    switch( varargin{1} ) 
        ...
        case 'write'
            error(generatemsgid('TODO'),'TODO:  The "write" command for wl_nodesConfig() has not been implemented yet.');    
        otherwise
...
function nodeInfo = wl_nodesReadAndValidate( fid )
    ...
    if ( ~isfield(nodesStruct, 'serialNumber') | ~isfield(nodesStruct, 'ID') | ~isfield(nodesStruct, 'ipAddress') )
        error(generatemsgid('InputFailure'),'Does not have required fields: "serialNumber", "ID", and "ipAddress" ') 
    end         
    ...
    for n = 1:(nodeSize(2))
        ...
        if ( ~eq( length( addrChars ), 4 ) )
            error(generatemsgid('InputFailure'),'IP address of node must have form "w.x.y.z".  Provided "%s" ', nodesStruct(n).ipAddress)         
        end

        if ( (addrChars(4) == 0) | (addrChars(4) == 255) )
            error(generatemsgid('InputFailure'),'IP address of node %d cannot end in .0 or .255', n)         
        end
    ...
    for n = 1:(nodeSize(2) - 1)
        for m = n+1:(nodeSize(2))
            if ( strcmp( nodesStruct(n).serialNumber, nodesStruct(m).serialNumber ) )
                error(generatemsgid('InputFailure'),'Serial Number must be unique.  Nodes %d and %d have the same serial number %s', n, m, nodesStruct(n).serialNumber)
            end
            if ( strcmp( nodesStruct(n).ID, nodesStruct(m).IDUint32 ) )
                error(generatemsgid('InputFailure'),'Node ID must be unique.  Nodes %d and %d have the same node ID %d', n, m, nodesStruct(n).ID)
            end
            if (nodesStruct(n).ipAddressUint32 == nodesStruct(m).ipAddressUint32)
                error(generatemsgid('InputFailure'),'IP Address must be unique.  Nodes %d and %d have the same IP Address %s', n, m, nodesStruct(n).ipAddress)
            end    
        end
    end
